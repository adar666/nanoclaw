...
- ~~OCR quality fallback if the agent's own vision-based reading of a scanned page proves insufficient in practice (a dedicated OCR engine, if ever needed, is out of scope per SPEC.md's no-new-OCR-engine decision — revisit only if the vision-fallback approach demonstrably fails).~~ **Shipped by spec-2-1 (2026-08-24, incl. step-04 patch round)**: `save_document`'s scanned-PDF path now runs `tesseract.js` server-side on the page-1 render — English and Hebrew (`eng+heb`, operator decision) — replacing agent vision-transcription as the primary extraction mechanism for that one case. Still page-1-only, PDFs only (unchanged scope). Agent vision-transcription is preserved only as the Ask-First fallback, now triggered by a real near-empty threshold (a trimmed-character-count floor, not just an exact-empty check — a stray single character from a noisy/blank page no longer sails through as if it were real content) rather than exact-empty alone. `SPEC.md`'s Constraints and CAP-1 are amended accordingly; see spec-2-1's own Spec Change Log for the full override rationale, the eng+heb and near-empty-threshold decisions, and the live-verified Bun-runtime integration gaps (worker module resolution, traineddata cache path, unbounded OCR call) that don't apply to any of this feature's other, already-shipped dependencies. Not a full, unconditional closure of the original deferred item — non-English/non-Hebrew scanned text and multi-page scanned PDFs remain out of scope, falling through to the same Ask-First vision fallback (or declined outright for multi-page, per existing page-1-only behavior).
...
- A saved signature has no OKF concept file or `memory/index.md` entry (unlike `save_document`'s AD-6 shape) — by design, per the frozen spec's storage shape (just `memory/signatures/<name>.png`). This means a signature is only discoverable by an agent that already knows/guesses its exact name, not via the recall/list mechanism `list_documents` provides for saved documents. Resolved by Story 1.7: `signatureName` resolution lists the actual `.png` base names present in `memory/signatures/` on a miss — a lightweight, on-demand discovery mechanism, not a dedicated `list_signatures` tool or a `memory/index.md` entry; considered sufficient, not revisited.

**From `spec-1-7-stamp-a-saved-signature-into-a-saved-pdf.md` (code review — blind-hunter/edge-case-hunter/verification-gap):**

- Stamped-image page rotation isn't consulted — a rotated scanned/printed page's `/Rotate` entry is ignored by both this story's image draws and the pre-existing text-fill draws, so a stamp can land in the wrong visual position/orientation relative to what a viewer actually renders for a rotated page. Pre-existing limitation, not new to this story; revisit if a rotated real-world PDF surfaces the issue in practice.
- No test decodes the embedded signature image's actual pixel content and diffs it against the source PNG — coverage is placement/non-distortion (content-stream geometry) only, not pixel-fidelity of the embed round-trip. `pdf-lib`'s `embedPng` is a well-established, widely-used encode/decode path; considered low-risk enough not to warrant the extra fixture complexity now.
- Success-message wording differs cosmetically across the three PDF stamp targets ("Stamped ... into field" / "Drew ... after line" / "Drew ... at the position") — only the shared `FILL_SUCCESS_MARKER` substring is guaranteed consistent. Not worth unifying now; revisit if it ever causes real confusion.
- `signatureName` combined with both `fieldName` and `lineNumber` in one call isn't explicitly tested — behavior is unchanged from the pre-existing (already-silent) priority order `fillPdf` used before this story, so not a new gap, just an untested pre-existing one.

**From `spec-1-8-stamp-a-saved-signature-into-a-saved-docx.md` (code review — blind-hunter/edge-case-hunter/verification-gap):**

- No test byte-compares the embedded signature image in `word/media/imageN.png` against the resolved signature source PNG — coverage confirms the embedded bytes decode as a valid PNG, not pixel-for-pixel fidelity of the embed round-trip. Same posture as Story 1.7's PDF-side deferral.
- No atomicity/failure-injection test for the three-part OOXML write (media + relationship + content-type) — the "all three or none" guarantee is a code-structure property (in-memory zip edits, single `generateAsync`/`writeFileSync`), not adversarially tested against a simulated partial failure.
- `nextDocPrId`'s collision-avoidance only scans `word/document.xml` — a `docPr`/`cNvPr` id already used in a header/footer/footnote part isn't considered. Low real-world risk; revisit if a real collision surfaces.
- Inserted signature/text runs carry no `<w:rPr>` (no `w:rtl`, no font) — inherited from the pre-existing `insertRunAfterParagraph` convention (Story 1.4), not new to this story, but this story doubles the surface (image run + optional text run) where a missing bidi hint could visually misorder next to Hebrew paragraph text in an RTL form.
- No page-width-equivalent clamp on the computed EMU width for an extreme-aspect-ratio signature on the `.docx` path (unlike the PDF path's off-page-edge decline, Story 1.7) — `.docx` inline content has no absolute page-position concept to clamp against the same way. Not fixed; revisit if it proves visually disruptive in practice.
- `appendRunXmlIntoCell`/`appendRunXmlAfterParagraph` are near-verbatim duplicates of the pre-existing `insertRunIntoCell`/`insertRunAfterParagraph`, differing only in precomputed-vs-built run XML — a future fix to either insertion-point algorithm needs remembering to apply in both places. Minor drift-prevention nice-to-have, not urgent.
