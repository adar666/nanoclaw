...
- **Deletion/cancellation — built, 2026-08-18** (bounded change, no new architecture-spine revision): `delete_calendar_event`, same `calendar.ts` file, same `CALENDAR_IDS`/`searchEvents`/AD-8 plumbing as the other three tools. No new architectural decision needed — the only new mechanic is a `confirm: boolean` gate: omitted/false resolves and previews the target event without deleting it; `true` issues the real `DELETE`. This is agent/persona-level trust (SKILL.md instructs always confirming via `ask_user_question` first), not a server-side state machine tying the two calls together — same trust model as AD-7's disambiguation.
