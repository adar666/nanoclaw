...
- **CAP-5's single-occurrence non-goal, narrowed (2026-08-19, web-verified against Google's own docs — retro action item 4):** confirmed `list_calendar_events` does surface instance-level `eventId`s (`singleEvents=true`, per Story 2.1's own investigation) — so both tools *can* be called against one. Split by tool:
  ...
  - **`delete_calendar_event` (DELETE) — RESOLVED, live-verified 2026-08-19.** Docs alone were ambiguous (see history below), so live-tested against a real recurring event ("בדיקה2", weekly Mondays) via `delete_calendar_event` targeting the first occurrence's instance `eventId`: the delete confirmation card correctly showed just that occurrence's real date/time, and after confirming, **only that one occurrence was removed — the rest of the weekly series was left intact**, matching PATCH+cancelled's documented effect exactly. Raw `DELETE` on an instance `eventId` is confirmed safe. No refuse/block/convert-to-PATCH needed — current silent-allow behavior is correct, same conclusion as `update_calendar_event`. (History: multiple docs sources recommend PATCH+`cancelled` over raw `DELETE` as the *documented pattern*, but none stated `DELETE` actually fails on an instance — that ambiguity is what the live test resolved.)
