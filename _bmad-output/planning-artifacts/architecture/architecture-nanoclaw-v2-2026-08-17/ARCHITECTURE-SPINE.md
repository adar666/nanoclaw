...
- **CAP-5's single-occurrence non-goal, narrowed (2026-08-19, web-verified against Google's own docs — retro action item 4):** confirmed `list_calendar_events` does surface instance-level `eventId`s (`singleEvents=true`, per Story 2.1's own investigation) — so both tools *can* be called against one. Split by tool:
  - **`update_calendar_event` (PATCH) — resolved, no action needed.** Google's own recurring-events guide (`developers.google.com/workspace/calendar/api/guides/recurringevents`) documents PATCHing a single instance as the correct, intended way to create an "exception" for that occurrence — this is desired behavior, not a gap. Silently allowing it (current behavior) is *correct*, and the CAP-5 non-goal was never really at risk here — a user asking to "move just this Thursday" and getting exactly that is the right outcome, not a bug to guard against.
  - **`delete_calendar_event` (DELETE) — genuinely still open.** Multiple sources (Google's own guide, third-party docs) consistently recommend PATCHing an instance's `status` to `"cancelled"` to cancel a single occurrence, rather than calling `events.delete` directly on the instance `eventId` — but none of them state outright that a raw `DELETE` on an instance *fails* or misbehaves, only that it isn't the documented pattern. This is genuinely unresolved from docs alone. **Before deciding**, live-test `delete_calendar_event` against a real instance `eventId` of a real recurring event and observe: does it cleanly cancel just that occurrence (matching PATCH+cancelled's effect), or does something else happen (whole-series deletion, an error, or a series exception it doesn't format correctly)? Decide refuse/allow/convert-to-PATCH only after that observation — not blind, matching this project's own established live-verification discipline.
