...
- 2026-08-18 (retro follow-up, epic-1-retro-2026-08-18.md action item AI-3, live verification): the "verify live once implemented" acceptance criterion, open since cal-1.2's live-verification round, is now closed. A real `list_calendar_events` call against the household chat returned real events on both `uriel`'s and `devorah`'s calendars — including pre-existing real appointments (a doctor visit, personal meetings) alongside the two test events created during cal-1.2's own live verification — confirmed by reading the actual `messages_out` row (`data/v2-sessions/ag-2146c1ff-.../sess-1785686864517-55gbpn/outbound.db`, seq 251, 2026-08-18T08:40:21Z), not just the agent's self-report.
