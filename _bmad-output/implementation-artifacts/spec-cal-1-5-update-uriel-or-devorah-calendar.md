...
- 2026-08-18 (retro follow-up, epic-1-retro-2026-08-18.md action item AI-3, live verification): the "verify live once implemented" acceptance criterion, open since cal-1.2's live-verification round, is now closed. A real `update_calendar_event` call against the household chat resolved the event "בדיקת יומן - אוריאל" by `eventId` (taken from the same live `list_calendar_events` call above), PATCHed its title to "בדיקת יומן - אוריאל (עודכן)", and returned a real Google `htmlLink` confirming the change landed — confirmed by reading the actual `messages_out` row (`data/v2-sessions/ag-2146c1ff-.../sess-1785686864517-55gbpn/outbound.db`, seq 251, 2026-08-18T08:40:21Z), not just the agent's self-report.
