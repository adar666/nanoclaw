...
- 2026-08-17 (post-merge architecture pivot, user-confirmed and live-verified — see `ARCHITECTURE-SPINE.md`'s memlog): connecting Google Calendar under household's OneCLI identity turned out to only be possible once per *project*, not once per agent identity (no `--agent` scoping exists anywhere in OneCLI's CLI). The frozen Intent/Boundaries above describe "this agent's own calendar (`calendarId=primary`)" as the sole scope of this story — that stays literally true for the `uriel` case, but the tool was extended with a `calendar` (`uriel`|`devorah`) argument so Devorah's calendar (reached via her own Google-native calendar-sharing grant to the same connected account, not a second OAuth connection) is handled by the SAME tool rather than a separate Story 1.4 relay mechanism, which turned out to be unnecessary — there is no second identity to relay to or from. **Live-verified end-to-end**: `calendar: 'uriel'` created a real event with a real `htmlLink`; `calendar: 'devorah'` correctly failed with a real Google `403 requiredAccessLevel` (writer access needed) before she completed the sharing step — confirming both the success and the expected-failure path work exactly as designed. Original Stories 1.1/1.4/1.6 (cross-agent wiring + relay) are obsolete; see `epics-google-calendar.md`'s pivot note.
