...
CAP-3 ("fill row X with value Y") must resolve to one of three mechanisms depending on the source file, auto-detected — the user never picks a mode explicitly.
...
2. If the file is `.pdf` and it has AcroForm fields matching the target → form-field path.
3. Otherwise (`.pdf` with no matching form field, or a free-text line in either file type) → overlay path.
