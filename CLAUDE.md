...
- **A host-side TS change (`src/**`) needs `pnpm run build` + a service restart (`launchctl kickstart -k gui/$(id -u)/<service-label>` on macOS) to take effect — a container rebuild does nothing for it.** A container-side change (`container/agent-runner/src/**`, `container/skills/**`) needs `./container/build.sh` + the same service restart — a host-only rebuild does nothing for it. Two separate rebuild+restart paths; changing one side and only restarting for the other is a real, easy-to-make mistake. **Correction found live (2026-08-16):** on an install with `NANOCLAW_HARDENED_IMAGE=true` (`.env`), `container/agent-runner/src` is bind-mounted read-only into the container at spawn time (`src/container-runner.ts`'s `/app/src` mount) — a plain TS source edit under that path needs neither a rebuild nor a service restart, only a fresh container spawn (which happens automatically on the next inbound message once no container is currently running for that group). `./container/build.sh` on a hardened install only re-applies the CLI-tools overlay; it does NOT rebuild `/app/node_modules` unless `container/agent-runner/bun.lock` actually changed (see the script's own `OVERLAY`/lock-SHA check). Don't assume a rebuild was necessary — or that one actually happened — without checking which mode the install is in. **Correction found live (2026-08-19):** the same mistake happened again in a different shape — a container image had already been rebuilt from an unmerged feature branch (containing both host and container changes) mid-development, and after fixing a bug and rebuilding the *container* image again, the corresponding *host* CLI verbs (`ncl groups config add-calendar`/`remove-calendar`, added in the same branch) still didn't exist at runtime until `pnpm run build` + a separate service restart ran — confirmed by `dist/cli/resources/groups.js`'s mtime predating the source changes by days. When a change touches both `src/**` and `container/agent-runner/src/**` in one commit, both rebuild+restart paths are needed, not just the one that happens to be top of mind. **Correction found live (2026-08-19):** the mount described above is not actually gated by `NANOCLAW_HARDENED_IMAGE` — `src/container-runner.ts`'s `mounts.push({ hostPath: agentRunnerSrc, containerPath: '/app/src', readonly: true })` (and the analogous `container/skills` mount) run unconditionally in this codebase's current state, on every install regardless of that env var. A container-side source edit (`container/agent-runner/src/**`, `container/skills/**`) needs only a fresh container spawn (`ncl groups restart`, or the automatic respawn on next inbound message) on *any* install — `./container/build.sh` is only load-bearing for Dockerfile-level changes (apt packages, CLI tools, entrypoint) or an actual `bun.lock` change. Verify this against the current `container-runner.ts` before relying on it again, in case the gating comes back in some other form.

- **The MCP-tools stdio server (`container/agent-runner/src/mcp-tools/index.ts`, spawned as the `nanoclaw` MCP server per `src/container-runner.ts`'s `mcpServers` config) runs in a SEPARATE process from `index.ts`'s own `main()` — with its own independent module registry, and therefore its own independent `config.ts` `_config` singleton.** `main()` calls `loadConfig()` early; that has zero effect in the MCP-tools subprocess, which never called it itself before this was found. Any MCP tool handler that calls `getConfig()` (e.g. `calendar.ts`'s `calendarConfigHooks`, added for the config-driven calendar registry) throws `"Config not loaded"` on every single call, deterministically, not intermittently — found live in production (2026-08-19) after this exact code shipped: every calendar tool call failed for hours, the agent tried a container restart on its own initiative (which structurally can't fix a subprocess-boundary gap), and only a fix inside `mcp-tools/index.ts` itself (calling `loadConfig()` there too, before `startMcpServer()`) resolved it. `index.wiring.test.ts` already guards a *different* gap in this same subprocess boundary (AD-15's env-inheritance fix); `mcp-tools/index.wiring.test.ts` (new) is the analogous structural guard for this one. The general lesson: a comment claiming "safe because X already ran by the time this executes" needs to name which *process* X ran in, not just which function — same-codebase, cross-process assumptions are exactly the kind of thing no unit test (which runs everything in one process) can catch.

- **Each agent group's `/workspace/agent/memory/` is fully isolated — a persona instruction that names a specific memory file path (e.g. `groups/household/memory/household/people.md`) only resolves for the group that actually owns that file.** A SKILL.md instruction shared across multiple agent groups (the `calendar` container skill, mounted into every group) that hardcodes one group's memory path silently fails for every *other* group — found live (2026-08-19): asked to resolve a household member's email, `dm-with-uriel` (a personal DM agent, not the `household` group) had no `people.md` at all in its own workspace and asked the user for the email instead of finding it, even though the email was recorded in `household`'s own memory the whole time. The fix that doesn't create a sync problem is a **read-only mount** of the source group's file into the other groups (`ncl groups config add-mount`, `AdditionalMountConfig`) — never a copied duplicate, which drifts the moment either copy is edited. A mounted file lands under `/workspace/extra/<containerPath>` (per `container-runner.ts`'s mount convention), a different path from a group's own native `/workspace/agent/memory/...` — a shared persona instruction referencing "the household people file" needs to name *both* real paths and let the agent check whichever exists in its own workspace, since the same logical file sits at a different path depending on which group is asking.

- **A rejected `additionalMounts` entry is `WARN`-logged with the real reason in `logs/nanoclaw.error.log` (`"Additional mount REJECTED"`, `src/modules/mount-security/index.ts`) — check that log *before* exec'ing into a container to look for a missing file.** Found live (2026-08-19): a new mount (see the isolated-memory pitfall above) silently didn't appear inside the container because its host path wasn't on the operator's `~/.config/nanoclaw/mount-allowlist.json` — diagnosed backwards, by `docker exec`-ing in and finding the file missing first, only checking `nanoclaw.error.log` afterward and finding the exact rejection reason had been there the whole time. This is exactly the troubleshooting order this file's own table already prescribes ("Host logs: `nanoclaw.error.log` first") — a reminder that the prescribed order is worth following literally, not just for messaging/delivery bugs. The allowlist itself is loaded lazily and `mtime`-cached per-process (`src/modules/mount-security/index.ts`), so a fresh entry needs a service restart to actually take effect, same as any host-side config change.
...
- **A purpose-built MCP tool sitting right next to `create_calendar_event`/etc. in the agent's own tool list can still lose to the general-purpose `ncl` CLI, even fresh after `/clear` — the agent doesn't reliably check "is there already a tool for exactly this" before reaching for `ncl help`.** Live-tested (2026-08-19), twice: after shipping the new `add_calendar` self-mod tool and rewriting the calendar `SKILL.md` (v1.11.0) to present it as the primary path, a real "תוסיף לעצמך יומן..." request to Yulanda (`dm-with-uriel`) went straight to `ncl groups config add-calendar` (the old CLI-only route, needing a manual follow-up `ncl groups restart`) — `add_calendar` never invoked, confirmed via `outbound.db` (zero `add_calendar` rows, only `cli_request`/`groups-config-add-calendar`). **First hypothesis — a stale resumed session not re-reading the updated SKILL.md — was disproven by a same-session retest**: `/clear`'d the group, confirmed `container/skills/calendar/SKILL.md` v1.11.0 was genuinely mounted (`docker exec`), asked again with a different test name — the agent's *very first* move was `ncl help` → `ncl groups help` → `ncl groups config help`, methodically discovering the CLI recipe via `ncl`'s own help system, and it went with that recipe without ever seemingly consulting its own MCP tool list for a more specific match. `container/agent-runner/src/mcp-tools/server.ts`'s `registerTools`/`ListToolsRequestSchema` has no filtering — `add_calendar` genuinely was in the tool list both times, not a registration bug. This reads as a persona/prompting-weight issue (the agent's own `ncl` CLI instructions are detailed and self-contained — "here's exactly how to call `config add-calendar`" — while noticing a same-purpose MCP tool exists requires it to scan its own tool list first, which it doesn't reliably do), not a code bug — **still open, no fix applied**. If this class of thing matters enough to chase, the next step is a persona-level instruction ("before using `ncl` for something, check whether a purpose-built tool already exists for it") rather than a calendar-specific patch, since the same failure mode would apply to any future self-mod action.

### BMad planning discipline

- **A spec's Deferred/Boundaries section saying "confirm X before shipping Y" is not self-executing — it needs to become an explicit checklist item in whichever *later* story actually ships Y, not something left to be remembered.** `ARCHITECTURE-SPINE.md`'s Deferred section flagged, before Story 2.2 (recurring events) shipped, that `list_calendar_events`' `singleEvents=true` behavior needed confirming and that `update_calendar_event`/`delete_calendar_event`'s handling of a recurring-instance `eventId` needed an explicit decision. Story 2.2 shipped without either happening — nothing in the single-story build loop re-reads a Deferred section before starting the next story that might resolve it, so the note just sat there until an epic-level retrospective caught it (closed in `_bmad-output/implementation-artifacts/epic-2-retro-2026-08-18.md`'s action item 4, resolved by web-verification for `update_calendar_event` and one live test for `delete_calendar_event`). When authoring a story spec, check the epic's own architecture spine's Deferred section for any "confirm before shipping this" note that this story is the one shipping — and carry it into the spec's own Tasks & Acceptance as a real item, not an assumption it'll be remembered.
...
| `eval/` | Agent Evaluation Harness — spins up the real per-agent-group container, drives scripted scenarios through it, judges deterministically or via a second Claude call, reports results. Run with `pnpm eval run <scenario-set-name>` (e.g. `pnpm eval run guest-resolution`, `pnpm eval run shared-context`) — real container, real Claude call, real tokens spent; get operator go-ahead first. Writes `eval/reports/<run-id>/report.json`. `pnpm eval sweep` is a separate, standalone command — finds and removes orphaned events left on the eval-test calendar by a crashed/interrupted run (a safe no-op when already clean); also real-container, get operator go-ahead first. `pnpm eval prune`/`pnpm eval teardown` (no real container/tokens spent, just local DB + filesystem state) reclaim eval-session state that accumulates indefinitely otherwise — `prune` deletes eval-managed sessions (DB row + on-disk `data/v2-sessions/` dir) for the eval/eval-judge groups but keeps the groups themselves provisioned; `teardown` does that plus removes the groups too, both recreated fresh on the next `pnpm eval run`/`sweep`. See `eval/teardown.ts`. Mirrors `scripts/`: imports host `src/**` modules directly, run via `tsx`, own standalone `eval/tsconfig.json` (not covered by the root `pnpm exec tsc --noEmit`). |
...
|----------|-------|------------|
| groups | list, get, create, update, delete, restart, config get/update, config add-mcp-server/remove-mcp-server, config add-package/remove-package, config add-calendar/remove-calendar, provenance-digest | Agent groups (workspace, personality, container config). `provenance-digest` federates task/self-mod/document provenance (specs 2-1..2-3) into one read-only summary — `src/modules/provenance-digest.ts` |
...
1. **`install_packages` / `add_mcp_server` / `add_calendar`** — changes to the per-agent-group container config in the DB (apt/npm deps, wire an existing MCP server, add/override a calendar registry entry — the last one, `add_calendar`, resolved a deferred-work.md finding on 2026-08-19: `config add-calendar` used to be a bare CLI-only verb with no auto-restart/notify UX, unlike its siblings). Single admin approval per request; on approve, the handler in `src/modules/self-mod/apply.ts` rebuilds the image when needed (`install_packages` only), writes an `on_wake` message, kills the container, and respawns via `onExit` callback. The on-wake message is only picked up by the fresh container's first poll — dying containers can never steal it. `container/agent-runner/src/mcp-tools/self-mod.ts`. Each approved change also appends one line to `groups/<folder>/self-mod-log.md` (`src/modules/self-mod/self-mod-log.ts`, capped, host-written) — mounted read-only into the group's own container so the agent can read but never tamper with its own self-mod audit trail (spec 2-2).
