...
- **A long-running resumed agent session does not re-read an updated `SKILL.md` — the agent keeps using whatever it already knows, even once a better tool exists and is genuinely callable.** Live-tested (2026-08-19): after shipping the new `add_calendar` self-mod tool and rewriting the calendar `SKILL.md` (v1.11.0) to present it as the primary path, a real "תוסיף לעצמך יומן..." request to Yulanda (`dm-with-uriel`, a session that had been running across this entire epic-2 testing marathon) went straight to `ncl groups help config add-calendar` then `ncl groups config add-calendar` — the *old* CLI-only route, requiring a manual follow-up `ncl groups restart` — with `add_calendar` never invoked at all, confirmed by grepping `outbound.db` for both action names (only `cli_request`/`groups-config-add-calendar` rows, zero `add_calendar` rows). Ruled out a deploy bug first: `docker exec`'d into the actual running container and confirmed `/app/skills/calendar/SKILL.md` really was v1.11.0 with `add_calendar` mentioned — the file was correct and mounted, the agent just never re-read it, most likely because the container process restarted (fresh MCP tool list) but the underlying Claude Agent SDK conversation was *resumed*, not started fresh, so whatever system-prompt-composed skill content it originally saw predates the edit. Matches this file's own repeated `/clear`-needed pattern for stale SKILL.md elsewhere in this epic, but had never been written down as its own pitfall until now. **The fix that actually worked: `ncl groups restart` alone is not enough to make an agent re-learn a new capability — the user needs to `/clear` (or otherwise start a fresh conversation) with that specific group before re-testing a newly-added tool for real.** When live-verifying a new capability against an existing long-running session, `/clear` it first, or the test will silently exercise stale behavior instead.
