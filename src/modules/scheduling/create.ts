...
/**
 * Who/what triggered a task series' creation, fixed once at creation time and
 * never regenerated by a later recurrence fire (`insertRecurrence` copies
 * `content` verbatim). `requesterUserId` stays structurally present but always
 * absent for now — `CallerContext` carries no resolved end-user identity at
 * this layer (see spec-2-1-task-reminder-provenance.md's Design Notes).
 */
...
export function createScheduledTask(
  ...
  options?: { status?: 'pending' | 'paused'; originSessionId?: string | null; provenance?: TaskProvenance },
